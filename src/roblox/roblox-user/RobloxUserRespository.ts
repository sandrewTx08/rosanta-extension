...
export default class RobloxUserRepository {
  ...
  isItemOwndByUser(userId: number, itemType: number, itemTargetId: number): Promise<boolean> {
    return fetch(`/v1/users/${userId}/items/${itemType}/${itemTargetId}/is-owned`).then(
